...
class OtherSettingController extends BaseController
{

    //
    public function homepageCenterBannerSettingAction(Request $request)
    {
        $bannerSetting = $this->getSettingService()->get('page_homepage_banner', array());  //获取questions配置项的数据  setting表
//var_dump($questionsSetting);exit();
        if (empty($bannerSetting)) {
            $default = array(
                'testpaper_answers_show_mode' => 'submitted',
            );
            $bannerSetting = $default;
        }

        if ($request->getMethod() == 'POST') {
            $bannerSetting = $request->request->all();
//            var_dump($bannerSetting);exit();
            $this->getSettingService()->set('page_homepage_banner', $bannerSetting);
            $this->getLogService()->info('admin/system/', 'admin_setting_homepage_center_banner_setting', '更新首页中部banner设置', $bannerSetting);
            $this->setFlashMessage('success', '首页banner设置已保存！');
        }

        return $this->render('admin/system/homepage-center-banner-setting.html.twig',array(
    ...
    //
    public function testDateSettingAction(Request $request)
    {
        $questionsSetting = $this->getSettingService()->get('page_homepage_banner', array());  //获取questions配置项的数据  setting表
//var_dump($questionsSetting);exit();
        ...
        return $this->render('admin/system/homepage-center-banner-setting.html.twig');
    }
